...
| Progress of an in-flight action of unknown length | `ProgressBar`                                               |
| Popover listing several notes, each with a link   | `InfoListPopover`                                           |
...
| `InfoListPopover` | Trigger button opening a popover that lists several notes, each with its own external link | `entries` (`title`/`description`/`linkLabel`/`linkUrl`), `triggerLabel`, `heading`, `emptyText`, `testId` | 1 file (HSM V2) | Use `HelpIcon` for one sentence + at most one link; this is for a *list* where every entry carries its own link. Links open in a new tab with `rel="noopener noreferrer"`. |
...
| `ProgressBar`    | Indeterminate bar + caption for an in-flight action inside a form | `label`, `className`, `testId`              | 1 file (HSM V2)         | Indeterminate only, on purpose — add a determinate mode only when a real percentage exists. Use instead of raw MUI `LinearProgress`. For a whole page/section use `Layout/Loading`. |
...
| `HelpIcon`            | Info icon with heading + "Learn more" tooltip                                    | `helpData`                                                  | 7 files            | Coupled to the `HelpData` shape in `common/HelpData`. One heading, one link — for several notes each with their own link use `InfoListPopover`. |
